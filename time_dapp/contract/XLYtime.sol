...
contract Time{
    ...
    constructor(uint32 initalSupply) public {
        ...
        projects[0].dateList.push(1634313600);
        projects[0].dateList.push(1634400000);
        ...
        projects[1].dateList.push(1634313600);
        projects[1].dateList.push(1634400000);
        ...
        _joinUser2.joinDate=1634313600;
    ...
    function submitProjectInformation(bytes32 _name,string _description,uint _startApplyTime,uint _endApplyTime,uint[] _account,uint[] _dateList) public{
        ...
        projects.push(project);
        frozenCoin(msg.sender,PROMOTERFROZEN);
    ...
    // 发起人资格认证 (没用到，但是先留着)
    function promoterCertification(address _address)public view returns(bool,uint) {
        // 包含自定义结构体作为参数的函数均需要声明为internal
        if(usersMap[_address].timeCoin>=PROMOTERFROZEN){
            return (true,PROMOTERFROZEN);
        }else{
            return (false,PROMOTERFROZEN);
        }
    ...
    function setVoteProjectApply(uint ID,int result) public returns(bool){
        // 还需要检查该项目的voteProjectApply中是否已经存在该审核人的审核结果
        require(result==0 || result==1 || result==-1,"error:voteResult值错误:投票结果值不在(0,-1,1)中");
        for(uint i=0;i<projects[ID].voteProjectApplyCount;++i){
            if(projects[ID].voteProjectApply[i].voteReviewer==msg.sender){
                projects[ID].voteProjectApply[i].voteResult=result;
                return true;
            }
        }
        return false;
    ...
    function setVoteProjectResult(uint ID,int result) public returns(bool){
        // 还需要检查该项目的voteProjectResult中是否已经存在该审核人的审核结果
        require(result==0 || result==1 || result==-1,"error:voteResult值错误:投票结果值不在(0,-1,1)中");
        for(uint i=0;i<projects[ID].voteProjectResultCount;++i){
            if(projects[ID].voteProjectResult[i].voteReviewer==msg.sender){
                projects[ID].voteProjectResult[i].voteResult=result;
                return true;
            }
        }
        return false;
    ...
    //发起人修改参与者应得时间余额
    function setJoinUserDuration(uint _ID,address _joinAddress,uint _joinDate,uint _joinDuration)public{
        require(isJoinedProject(_ID,_joinAddress,_joinDate),"该用户未参加该项目该天的活动");
        require(msg.sender==projects[_ID].promoterAddress,"该用户不是该项目的发起者！无法修改！");
        for(uint i=0;i<projects[_ID].joinUsersCount;i++){
            if(projects[_ID].joinUsers[i].joinAddress==_joinAddress && projects[_ID].joinUsers[i].joinDate==_joinDate){
                projects[_ID].joinUsers[i].joinDuration=_joinDuration;
                return ;
            }
        }
    }
    ...
    //发起人修改参与者备注
    function setJoinUserRemark(uint _ID,address _joinAddress,uint _joinDate,string _remark)public{
        require(isJoinedProject(_ID,_joinAddress,_joinDate),"该用户未参加该项目该天的活动");
        require(msg.sender==projects[_ID].promoterAddress,"该用户不是该项目的发起者！无法修改！");
        for(uint i=0;i<projects[_ID].joinUsersCount;i++){
            if(projects[_ID].joinUsers[i].joinAddress==_joinAddress && projects[_ID].joinUsers[i].joinDate==_joinDate){
                projects[_ID].joinUsers[i].remark=_remark;
                return ;
            }
        }
    }
    ...
    //发起人提交 公益项目确认
    function submitProjectFix(uint _ID) public returns(bool){
        if(projects[_ID].state!=ProjectState.FINISHED){
            //该公益项目不处于已结束状态，不可提交该公益项目的参与结果！
            return false;
        }else{
            projects[_ID].state=ProjectState.WAITINGFIX;
            return true;
        }
    }
    // ====================================个人测试代码====================================
